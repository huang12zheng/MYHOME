...
function getFileList(filepath,itype,action,caseaction)'itype文件类型,action是否递归
  ...
  IF itype <> " " then '是文件
	  For Each file In folder.Files '遍历文件
		...
		ext = UCase(Right(file.Name, iTypeLength+1)) '获取扩展名
		If ext = "."&itype Then
		  'V3 20161106 修改getFileList,建立唯一增序索引-时间(名称没有增序的特点)
...
Sub step4'a生成dll列表
	stepstatus="step4"
	Call setListFileByitypeAndPath("dll","D:\递交包下载\unrar")
	HSRCPtradepath="D:\办公软件\hundsun209\hundsun209\Trade\Biz"
	Set ws=WScript.CreateObject("WScript.shell")
	ws.run HSRCPtradepath
	dllarray=getStringArrayByFile("DLLlist.txt")
	for each strline in dllarray
		if strcomp(trim(strline),"")<>0 then
			DLLfilename=getStringByLeftSplit(strline,"unrar",1)'0左,1右
			'msgbox HSRCPtradepath&DLLfilename
			call copyPathToPath(strline,HSRCPtradepath&DLLfilename)
		end if
...
Sub step5'a生成dll列表
	stepstatus="step5"
	Call setListFileByitypeAndPath("ini","D:\递交包下载\unrar")
	HSRCPtradepath="D:\办公软件\hundsun209\hundsun209\Trade"
	Set ws=WScript.CreateObject("WScript.shell")
	ws.run HSRCPtradepath
	dllarray=getStringArrayByFile("INIlist.txt")
	for each strline in dllarray
		if strcomp(trim(strline),"")<>0 then
			DLLfilename=getStringByLeftSplit(strline,"unrar",1)'0左,1右
			'msgbox HSRCPtradepath&DLLfilename
...
'获取文件夹下sql列表
	...
	s=getFileList(filepath,itype,"0",4)
	's=getFileList(filepath,"sql","0",2) 没有版本!!
	'MsgBox s
	'生成新的文件列表,可以命名有点不规范
	Call WriteTxtFile(itype&"List.txt",s)'调用过程(Sub)时不能使用括号 call'FileList'NewList
	Call sortFile(itype&"List.txt",1)
End Sub
sub setListFileByitypeAndPath(itype,filepath)'setListFileByitypeAndPath("ini","D:\递交包下载\unrar")
	itype=ucase(itype)
	s=getFileList(filepath,itype,"0",4)
	Call WriteTxtFile(itype&"List.txt",s)'调用过程(Sub)时不能使用括号 call'FileList'NewList
	Call sortFile(itype&"List.txt",1)
...
'获取文件夹下sql列表
	itype=ucase("sql")
	filepath = PDC_PATH'"D:\递交包下载\unrar"'
	s=getFileList(filepath,itype,"0",4)
	's=getFileList(filepath,"sql","0",2) 没有版本!!
	'MsgBox s
	'生成新的文件列表,可以命名有点不规范
...
function getUserName(sqlfilename)
	...
	select case left(lcase(sqlfilename),getSplitlen)
...
function getTNSnameArrayByUserName(UserName)
	...
	select case left(UserName,getSplitlen)
		...
		case "hs_fil"		StringOnTNSname=StringOnTNSname&TNSname(0)&vbCrLf
...
function createpdcListStringBySQLList
	...
	'iSplit="unrar\"
	'msgbox PDC_PATH
	'iSplit=substr(PDC_PATH,InStrRev(PDC_PATH,"\"))&"\"
	iSplit=PDC_PATH&"\"
	'msgbox isplit
	'typelen=Len(iSplit)
	'MsgBox 440
...
'PDC_PATH=InputBox("your_PDC_PATH:",,"D:\递交包下载\unrar")
PDC_PATH=InputBox("your_PDC_PATH:",,"D:\递交包下载\M201701110299-日终清算\集成-M201701110299-日终清算-阮哲恒-20170124-V2")
stepstatus=""
'msgbox "1"&pdc_path&"1"

'filepath = "D:\递交包下载"
if StrComp(PDC_PATH,"") <> 0 then
			step3'生成并执行pdc,暂时只生成,不执行
			setErrorInfoByExePCD'调试20161108
	end if